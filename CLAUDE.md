...
- **게임 기획서(최상위 문서)**: `C:\Users\siwon\Documents\게임 개발 기획서-gmae_project.docx`
  ...
  - 미구현 축: **패링**(투사체를 슬라이딩으로 튕김), **슬라이딩 개조**(Phase 4 해금의 실체).
    과열(過熱)은 2026-09-09에 들어갔다 (아래 Phase 3 참고)
...
- **Phase 1 (조작감) 완료** — 태그 `phase-1-movement`. 점프·수평 이동 모두 사용자 확인 받음.
  점프 최종값: `timeToApex 0.28`, `timeToFall 0.2832`, `apexBonusSpeed 0` (그대로다)
  **수평 이동 수치는 2026-09-10에 갈아엎었다** — 슬라이드 진입 게이트 때문이다.
  당시 값은 `maxSpeed 9` / `groundAccel 110` / `airAccel 100`,
  현재 값은 아래 "이동·과열 수치 재조정" 참고
...
- **Phase 3 (전투 코어) 진행 중** — 브랜치 `feature/world`
  ...
  - ✅ **적 AI 완료** (2026-09-07) — Play 모드 실측 검증. `EnemyConfig` + `EnemyAI`(enum FSM) + `Projectile`
    ...
    - **`detectRange`와 `loseRange`를 따로 둔다(8 / 12).** 같으면 경계선에서 추격/순찰이 매 프레임
      번갈아 바뀌며 덜덜 떤다
    - **`chaseSpeed 5`는 플레이어 `maxSpeed`(현재 12)보다 느리다.** 뿌리칠 수 있어야 슬라이드가 답이 된다.
  ...
  - ✅ **슬라이드 전투를 속도 기반으로 재정비** (2026-09-09) — Play 모드 실측 검증
    - **무적과 히트박스가 같은 값 하나(`PlayerController.AtSlideSpeed`)를 읽는다.** 판정 기준은
      슬라이드 '상태'가 아니라 **수평 속도**(`MovementConfig.invincibleSpeed`, 현재 16.
      **항상 maxSpeed보다 커야 한다** — 작으면 걷기만 해도 무적이다). 상태로 판정하면 슬라이드가 끝나는 순간(마찰·발판 이탈·
      점프·내리막 종료) 판정이 절벽처럼 사라져 적을 뚫는 도중에 맞는다. 속도는 서서히 줄기만 하므로
      그 경계가 없고 **공중까지 저절로 이어진다**(슬라이드 점프 공중 무적 0.57초 실측)
    - **무적을 `Update`에서 걸면 안 된다.** 아래 "오늘 겪은 것" 참고. `PlayerController.FixedUpdate`
      **끝**에서 건다
    - **넉백 중은 제외한다.** 적 몸통 넉백이 12로 임계값과 정확히 같아서, 맞고 밀려나는 것만으로
      판정이 켜져 때린 적을 되받아쳤다. 맞은 것이 공격이 되면 안 된다
    - **`EnemyConfig.hitstun` 0.25** — 넉백이 멎은 뒤부터 깎는 경직. 맞는 즉시 채우므로 벽에 부딪혀
      넉백이 일찍 끝나도 굳는 시간은 그대로다. 없으면 밀려나기를 끝내는 순간 `chaseSpeed`로 되붙어
      밀치기 → 슬라이드 연계가 성립하지 않는다. 실측: 넉백 0.445s → 경직 해제 0.705s
    - 실측: vx 9 → 무적 off / vx 20·-20 → on, 평지 슬라이드 31.50 → **11.50에서 해제**(0.40s
      = (32-12)/50), 넉백 vx 19.60에서 판정 **off**
  - ✅ **속도 비례 피해** (2026-09-09) — `AttackConfig.speedDamageReference`(32 = dashSpeed) +
    `speedDamageExponent`(2). 피해 = `damage x (때린 쪽 |vx| / 기준) ^ 지수`. 기준 0이면 꺼진다(기본)
    - **지수 1(선형)은 피해 차이가 속도 차이를 구조적으로 못 넘는다.** vx가 50→31로 1.6배 갈리면
      피해도 1.6배가 천장이다. 지수 2로 올려 쓸 수 있는 속도대(14~50)에서 **12.9배**가 벌어진다
    - 세로 속도는 안 센다 — 낙하 속도가 피해로 새면 그냥 떨어지기만 해도 세게 때리는 게 된다
    - `SlideAttackConfig` damage 1 → **1.25**. 평지 슬라이드 진입(32)이 1.24로 사실상 그대로라
      기준선은 안 흔들렸다. 벌어진 건 양 끝이다
    - 실측 (기준 32 / 지수 2 / damage 1.25):

      | vx | 49.88 | 41.88 | 31.88 | 23.88 | 15.88 | 13.88 |
      |---|---|---|---|---|---|---|
      | 피해 | 3.037 | 2.141 | 1.241 | 0.696 | 0.308 | 0.235 |

      HP 3 적 기준 경사 레인 한 번의 주행에서 **1방 / 2방 / 3방**으로 갈린다
  - ✅ **과열(過熱) 완료** (2026-09-09) — Play 모드 실측 검증. 기획서 "자원관리 : 슬라이딩 과부화".
    무한 슬라이딩을 막는 유일한 제동 장치다
    - **쌓는 기준은 `AtSlideSpeed`다.** 슬라이드 '상태'가 아니다 — 상태로 세면 슬라이드 →
      점프 → 슬라이드 연계의 공중 구간이 공짜가 된다(그동안에도 무적이고 적을 때린다).
      **이득 보는 구간과 대가 치르는 구간이 어긋나면 자원이 아니다.** 무적·히트박스·과열
      셋이 같은 값 하나를 읽는다
    - **자원의 모양은 게이지(0~1)가 아니라 스택이다.** 처음엔 게이지로 만들었다가 사용자
      요청으로 바꿨고, 바꾸니 코드가 줄었다 — 아래 "게이지를 스택으로" 참고
    - `MovementConfig`에 세 값. `heatMaxStacks 4`(0이면 과열 통째로 꺼짐) /
      `heatPerStack`(현재 0.32 — 평지 슬라이드가 무적 속도로 있는 시간과 같게 잡아
      **슬라이드 1회 = 1스택**을 유지한다. 유도는 아래 "이동·과열 수치 재조정") /
      `heatRecoverTime 1`
    - **타이머 하나가 양쪽을 다 센다.** 빠르면 올라가고 아니면 내려가다, 양 끝(`heatPerStack`
      / `-heatRecoverTime`)에서 칸을 하나 옮기고 0으로 돌아간다. 게이지 시절의 `_heat`(float)
      + `_overheated`(bool) + `heatResumeRatio`가 이 하나로 합쳐졌다
    - **`UpdateHeat`은 `FixedUpdate` 끝, 무적 거는 자리 바로 다음.** 같은 이유다(속도가 확정된 뒤)
    - **가득 차면 달리던 슬라이드도 끊는다.** 안 끊으면 긴 내리막 하나로 한계를 넘긴 채
      끝까지 미끄러져 "막힌다"가 체감되지 않는다. **속도는 안 건드린다** — 번 모멘텀은
      `momentumDecel`로 자연스럽게 빠진다. 의족이 지친 것이지 벽에 부딪힌 게 아니다
      (실측: 끊긴 순간 `IsDashing False` / `vx 21.2`로 계속 미끄러짐)
    - `HeatZone`(`Code/World/`) — 트리거 하나 + `rateMultiplier` 한 값이 기획서의
      "냉각과 과열" 축 전부다. 물웅덩이 0(안 쌓인다) / 뜨거운 지대 3. **배수는 쌓는 쪽에만
      곱한다** — 기획서 예시가 "생성량 조정"이고, 안 쌓이는 것만으로 웅덩이는 이미 쉼터가 된다.
      그래서 **물웅덩이에서 슬라이드하면 게이지가 내려가는 게 아니라 멈춘다**
    - 테스트 지형은 `TestMapBuilder`가 굽는다 — Zone3 활주로에 물웅덩이(x 214~222)와
      가열지대(x 228~236). 배수 확인은 순간이동만으로 되지만 **누적은 실제로 슬라이드해야 보인다**
    - 실측 (timeScale을 낮춰 관측 — 아래 "오늘 겪은 것" 참고):

      | 확인 | 결과 |
      |---|---|
      | 평지 슬라이드 1회 | 타이머 0.290s에 0.290 (배수 1:1) → 0.4에서 **정확히 1스택** |
      | 가열지대 통과 1회 | 같은 0.288s에 **2스택** (배수 3배). 한 번 지나면 4칸 중 2칸 |
      | 물웅덩이 / 밖 | `mul 0` / `mul 1`, 나가면 복구 |
      | 회복 | 0.96초 `4/4` 잠김 → 1.88초 `3/4` **해제**. 1초에 정확히 한 칸 |
      | 가득 찬 상태 | 버퍼를 채워도 `vx 0.00` / x가 204.00에서 **안 움직임** |
      | 주행 중 가득 참 | `IsDashing False`로 끊기고 `vx 21.2`는 유지 |

    - 🐛 **넉백 중 슬라이드가 과열을 통째로 건너뛰던 버그 수정** (2026-09-10, 사용자 제보
      "맞으면 무한 슬라이딩"). 원인은 과열이 아니라 넉백 플래그였다 —
      아래 "오늘 겪은 것"의 `EndKnockback` 항목 참고
  ...
  - ✅ **슬라이드 진입에 최고속도를 요구** (2026-09-10) — Play 모드 실측 검증.
    `MovementConfig.slideMinSpeedRatio 0.95` + `PlayerController.CanStartSlide`
    - 그전까지 슬라이드는 **제자리에서도 나갔다.** 기획서의 "가속해서 번 속도를 쓴다"가 아니라
      쿨다운만 기다리면 되는 순간이동기였다. 이제 `maxSpeed x 0.95`(= 11.4) 이상으로
      **달리고 있어야** 나간다 — 가속 구간 자체가 비용이 된다
    - **비율로 잡는 이유는 경사다.** 1.0(정확히 maxSpeed)을 요구하면 지면 흡착이 `vx`를 조금씩
      흔들어 램프 위에서 슬라이드가 씹힌다. 0.95는 그 흔들림만큼의 여유다
    - **연계는 안 깨진다.** 모멘텀을 안고 착지하면 `vx > maxSpeed`라 조건을 그냥 통과한다.
      슬라이드가 끝나는 속도도 `maxSpeed`(임계값 위)이므로 쿨다운 후 재진입도 그대로다 —
      무한 연쇄를 막는 것은 이 게이트가 아니라 여전히 **과열**이다
    - 실측: `vx 0.00`은 버퍼가 살아 있어도(0.184 남음) `IsDashing False` /
      `vx 11.00`(임계 아래) `False` / `vx 12.00`(maxSpeed) **발동** — `vx`가 32로 튀고
      `AtSlideSpeed`와 `Health.Invincible`이 동시에 `True`
  - ✅ **이동·과열 수치 재조정** (2026-09-10) — 위 게이트에 맞춘 재튜닝. **아직 확정 아니다**
    - `maxSpeed` 9 → **12** / `groundAccel` 110 → **40** / `airAccel` 100 → **35**.
      가속을 늦추고 천장을 올려 "최고속도까지 끌어올리는 시간"을 실제 비용으로 만든다.
      **가속이 110이면 게이트가 있으나 마나다** — 0.1초면 임계를 넘는다
    - `invincibleSpeed` 12 → **16**. maxSpeed가 12로 올라갔으니 12를 그대로 두면
      걷기만 해도 무적이 된다(위 항목의 경고가 그대로 발동한다)
    - `heatPerStack` 0.4 → **0.32**. 무적창 길이에 다시 맞춘 값이다 — 평지 슬라이드가
      `AtSlideSpeed`로 있는 시간이 `(dashSpeed - invincibleSpeed) / dashDecel`
      = `(32 - 16) / 50` = 0.32초라 **슬라이드 1회 = 1스택**이 유지된다.
      **산술로 맞춘 값이고 실측은 아직이다** — 적과 실제로 싸워보고 정한다
  - ⏸ **Phase 3 남은 것** — 보스
...
- **화면 떨림 해결 + 테스트 맵 전면 개편** (2026-09-08) — Play 모드 실측 검증
  ...
  - 폭 648유닛 / 구간 5개: `1 기본`(계단·천장 틈) `2 점프`(구덩이 2~6, 단 1~5)
    `3 슬라이드`(구덩이 9·7·7, 낮은 천장) `4 경사45`(4단 언덕·V자 골짜기·8단 큰 경사)
    `5 적`(사수 데크·엄폐물, 근접 2 + 사수 2). 구간 사이는 2유닛 틈으로 끊어 적이 넘어가지 못한다
  - **Zone5 경사 시험 레인** (x 578~646, **숫자키 6**) — 속도 비례 피해를 재는 곳. 8칸 45도 언덕에서
    내려와 **정지 더미 3기**(램프 발치 +2/+12/+24)를 차례로 스친다. 더미 간격이 곧 측정 지점이라
    다른 속도대를 보려면 `TestMapBuilder.DummyOffsets`만 고치고 다시 굽는다
  - 정지 더미는 근접 적을 복제해 `DummyConfig`(순찰·추격·감지 전부 0)와 HP 999만 갈아 끼운 것이다.
    콜라이더·레이어·접촉 히트박스 배선을 다시 만들 이유가 없다. 지워도 굽기가 다시 만든다
  - **`DebugStats`**(`Code/Utils/`, 굽기가 `DebugTools`에 자동으로 붙인다) — `vx / 판정 ON·OFF / HP`와
    `마지막 타격: 피해 X @ vx Y`를 화면에 띄운다. 속도 비례 피해는 체력 UI 없이는 눈에 안 보인다.
    HUD를 만들 때(Phase 5) 통째로 지운다
...
### 오늘 겪은 것 (반복하지 말 것)
- **Rigidbody2D를 `transform.position` 대입으로 순간이동시키면 다음 물리 스텝에 되돌아간다.**
  대입 직후 읽으면 옮겨진 값이 그대로 나와서 성공한 줄 알게 되는데, 물리 바디가 자기 위치를
  다시 써넣기 때문에 실제로는 아무 일도 안 일어난다. 증상은 "분명히 옮겼는데 엉뚱한 데서
  측정된 결과"다 — 검증용 순간이동에 이걸 쓰고 남의 자리에서 잰 수치를 놓고 한참 헤맸다.
  **`rb.position`에 쓸 것** (`DebugWarp`처럼 카메라도 같이 옮기려면
  `CinemachineCore.OnTargetObjectWarped`와 `Physics2D.SyncTransforms()`도 함께).
- **"지속 시간 대신 속도가 0이 될 때까지"로 끝나는 상태는, 다른 시스템이 그 속도를 붙잡으면
  영영 안 끝난다.** `Health`의 넉백은 `vx`가 0에 닿아야 풀리는데(의도된 설계 — 세게 맞으면
  오래 밀린다가 저절로 성립한다), `PlayerController.UpdateDash`는 넉백을 안 보고 매 스텝
  `vx`를 32로 덮어썼다. 그러면 `Health.FixedUpdate`가 영영 0에 못 닿아 **슬라이드가 끝날
  때까지 `IsKnockedBack`이 켜진 채로 붙어 있는다.**
  - `AtSlideSpeed`가 넉백을 제외하므로 그 슬라이드는 **무적도 히트박스도 과열도 전부 죽는다.**
    증상은 사용자가 본 "맞으면 과열이 안 쌓여서 무한 슬라이딩"이었지만, 같은 뿌리에서
    "맞고 나서 슬라이드하면 적을 못 뚫는다"도 같이 나고 있었다. **증상 하나만 보고 과열 쪽을
    고쳤으면 나머지 절반이 그대로 남았다.**
  - 해법은 `Health.EndKnockback()` — 슬라이드가 시작될 때 부른다. 넉백은 "남이 내 속도를
    정하는 동안"인데 슬라이드를 시작한 순간부터는 내가 정하기 때문이다.
  - 실측 (같은 주행 204 → 214.2): 고치기 전 `heat 0/4 / 타이머 0.000 / 넉백중 True / 판정 False`
    → 고친 뒤 `heat 1/4 / 타이머 0.340 / 넉백중 False / 판정 True`. 넉백 직후 슬라이드로
    더미 2기 관통 시 **더미 HP 999 → 997.2, 플레이어 HP 5 → 5 유지**.
  - **`ApplyHorizontal`은 이미 `IsKnockedBack`에 조기 반환하고 있었다.** 걷기는 막히는데
    슬라이드만 안 막히는 비대칭이 힌트였다 — 같은 상태를 보는 함수가 둘인데 한쪽만
    처리하고 있으면 의심할 것.
- **과열처럼 "쌓이고 식는" 값은 왕복 사이에 증거가 지워진다.** 슬라이드가 0.4초에 한 칸을
  쌓는데 회복이 1초에 한 칸이라, 슬라이드를 발동시키고 다음 eval에서 읽으면 **이미 0이다.**
  "안 쌓인다"로 오판하기 딱 좋다. `Time.timeScale`을 0.05까지 낮춰 회복 1초를 실제 20초로
  벌려두면 쌓이는 것만 골라 볼 수 있다 (읽고 나서 1로 되돌릴 것).
- **왕복 사이에 흐르는 것은 CLI 2초가 아니라 내 응답 지연까지다.** 이걸 2초로 잡고 스택을
  세다가 "한 번 슬라이드했는데 0"이라고 오판했다 — 실제로는 그 사이 10초 넘게 흘러 회복이
  다 돈 것이었다. **경과 시간을 가정하지 말고 `Time.time` 차이를 같이 찍어서 예상값을
  계산할 것** (`예상 = 최대 - floor(dt / heatRecoverTime)` 식으로 비교하면 지연과 무관해진다).
- **게이지를 스택으로 바꾸니 코드가 줄었다.** float 하나 + `_overheated` 플래그 +
  `heatResumeRatio`(히스테리시스)가 **int 하나 + 타이머 하나**로 합쳐졌다. 칸이 하나 비는
  순간이 곧 잠금 해제라 "얼마나 식어야 풀리는가"를 따로 정할 것이 없기 때문이다.
  게이지판에서는 이게 없으면 한계에서 한 프레임짜리 슬라이드 → 즉시 재과열이 반복됐다.
  - 덤으로 **버그 하나가 원리적으로 사라졌다.** 게이지판의 `Overheated`는 저장된 플래그라
    `UpdateDash`(FixedUpdate 앞) → `UpdateHeat`(뒤) 순서 때문에 `_heat`를 eval로 주입한
    프레임에 **슬라이드가 한 번 새어 나갔다**(실측 7.5유닛 이동). 스택판은 `_heat`를 그대로
    읽는 계산 속성이라 주입한 즉시 반영된다 (실측: 4/4에서 버퍼를 채워도 x가 204.00 그대로).
- **자원이 "몇 번 더 쓸 수 있는가"로 읽혀야 하면 게이지가 아니라 칸이다.** 서서히 줄어드는
  막대는 남은 횟수를 세게 해주지 않는다. 반대로 시간 비례로 **쌓는** 쪽은 그대로 뒀다 —
  긴 내리막이 평지 슬라이드와 같은 값이면 오래 빠른 것이 대가를 안 치른다.
- **물리에 걸리는 상태를 `Update`에서 켜면 최대 2 물리 스텝(20ms) 늦는다.** Unity는
  `FixedUpdate` → 물리 스텝(트리거 콜백) → `Update` 순으로 돈다. 슬라이드는 `FixedUpdate`에서
  시작되는데 무적을 `PlayerAttack.Update`에서 걸고 있었다. 물리 100Hz / 화면 60fps면 그 사이에
  트리거가 1~2번 도는데, 적의 몸통 히트박스는 `continuous`라 매 스텝 때린다. 증상은
  **"적이 붙었을 때 슬라이드로 빠져나가기"만 골라서 실패**하는 것 — 무적을 쓰라고 만든 상황에서만
  안 되니 "무적이 이상하다"로만 보인다. 상태를 만드는 쪽(`PlayerController`)이 `FixedUpdate` 끝에서
  직접 걸어야 한다.
- **`CinemachineBrain`을 꺼둔 채 씬을 저장하면 플레이해도 카메라가 그 자리에 멈춰 있다.**
  아래 "에디트 모드에서 카메라를 옮겨 캡처하려면…" 항목의 되돌리기를 안 해서 `m_Enabled: 0`이
  Main Camera 위치(x=392)와 함께 **커밋까지 갔다.** 증상은 "캐릭터가 안 보이고 화면이 고정"이다.
  굽기가 씬의 원본이므로 `TestMapBuilder.RestoreCamera()`가 매번 되돌린다.
- **`Health.Invincible`은 기록자가 하나뿐이라고 가정하고 있다.** `PlayerController.FixedUpdate`가
  매 스텝 덮어쓴다. 나중에 다른 무적(리스폰, 과열 잠금)이 생기면 카운터로 바꿀 것
  (코드에 `ponytail:` 주석으로 표시해 뒀다).
